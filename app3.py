...
def load_model():
    return joblib.load('knn_model.pkl'), joblib.load('scaler.pkl')
...
def plot_simple_eligibility(default_in_last_6months):
    # Determine eligibility based on default in last 6 months
    if default_in_last_6months == 1:  # If "Yes" (defaulted)
        prediction = "Not Eligible for Loan"
        color = 'red'
    else:  # If "No" (did not default)
        prediction = "Eligible for Loan"
        color = 'green'

    # Create a simple bar plot
    plt.figure(figsize=(6, 4))
    plt.barh(["Eligibility"], [1], color=color)
    plt.xlim(0, 1)
...
with st.form("loan_form"):
    ...
    if submitted:
        try:
            ...
            # Show loan eligibility based on default status in last six months
            plot_simple_eligibility(user_data["default_in_last_6months"])
                
        except Exception as e:
            st.error(f"An error occurred: {str(e)}")
            st.error("Please try again with different input values.")
